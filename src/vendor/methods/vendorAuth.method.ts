...
import { vendorDetails, vendorRegistrationInterface } from "../models/vendor.models";
...
const LoginVendorMethod = async (vendorLoginData: vendorRegistrationInterface) => {
  ...
  const passwordMatch = await bcrypt.compare(vendorLoginData.password, existingVendor.password);
...
const updateVendorMethod=async(newVendorData:vendorDetails)=>{
    
    const { Id, ...data } = newVendorData;

    const updateVendor = await prisma_client.vendor.update({
      where: {id},
      data,
    });
    return new SuccessResponse("Data Changed Succussfully",updateVendor);
}
...
export {RegisterVendorMethod,LoginVendorMethod,updateVendorMethod,fetchVendorMethod,deleteVendorMethod};
